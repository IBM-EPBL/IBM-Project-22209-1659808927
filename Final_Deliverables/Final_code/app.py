...
app.secret_key = 'a'
conn = ibm_db.connect("DATABASE=bludb;HOSTNAME=6667d8e9-9d4d-4ccb-ba32-21da3bb5aafc.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;PORT=30376;SECURITY=SSL;SSLServerCertificate=DigiCertGlobalRootCA.crt;UID=pht72647;PWD=IlLWoOw2JKv8XKkj",'','')
@app.route("/")
@app.route('/home')
def home():
...
def limit():
    ...
    if 'id' in session and 'email' in session:
        uid = session['id']
        exist = ibm_db.prepare(conn, 'SELECT * FROM limit WHERE uid = ?')
        ...
        if request.method == "POST":
              ...
              if exist == False:
                ...
                print("Updating")
                stmt = ibm_db.prepare(conn, "UPDATE limit SET limit = ? WHERE uid = ?")
...
def income():
    if 'id' in session and request.method == 'POST' and ('amount' in request.form) and ('income' in request.form) and ('expense' in request.form) and ('expensedate' in request.form )and ('category' in request.form):
            ...
            if sum >= data:
                #Limit Exceeded
                msg = Message('Limit Exceeded', sender='remymarthamartin@gmail.com', recipients=[email])
...
def verify():
    if request.method == 'POST':
        ...
        if account:
           ...
           ibm_db.bind_param(stmt, 1, email1)
           ibm_db.execute(stmt)
           msg = Message('Hello',sender ='remymarthamartin@gmail.com',recipients = [email1])
           msg.body = str(otp)
           mail.send(msg)
